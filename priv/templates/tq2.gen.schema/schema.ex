...
<% end %>    field :lock_version, :integer, default: 0

    timestamps()
  end

  @doc false
  def changeset(%Account{} = _account, %<%= inspect schema.alias %>{} = <%= schema.singular %>, attrs) do
    <%= schema.singular %>
...
<%= for k <- schema.uniques do %>    |> unsafe_validate_unique(<%= inspect k %>, <%= inspect schema.repo %>)
