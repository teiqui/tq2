defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>ControllerTest do
  ...
  def <%= schema.singular %>_fixture(_) do
    account = <%= inspect context.base_module %>.Repo.get_by!(<%= inspect context.base_module %>.Accounts.Account, name: "test_account")
    session = %<%= inspect context.base_module %>.Accounts.Session{account: account}

    {:ok, <%= schema.singular %>} = <%= inspect context.module %>.create_<%= schema.singular %>(session, @create_attrs)
  ...
  describe "index" do
    setup [:<%= schema.singular %>_fixture]
