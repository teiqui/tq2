defmodule Mix.Tasks.Tq2.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      mix tq2.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      mix tq2.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `tq2.gen.schema` for more information on
  attributes.
  """
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix tq2.gen.context must be invoked from within your *_web application root directory"
      )
    end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Phoenix.eval_from(paths, "priv/templates/tq2.gen.context/context.ex", binding)
      )
    end

    paths
    |> Mix.Phoenix.eval_from(
      "priv/templates/tq2.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        test_file,
        Mix.Phoenix.eval_from(paths, "priv/templates/tq2.gen.context/context_test.exs", binding)
      )
    end

    paths
    |> Mix.Phoenix.eval_from("priv/templates/tq2.gen.context/test_cases.exs", binding)
  ...
  def raise_with_help(msg) do
    ...
    mix tq2.gen.html, tq2.gen.json, tq2.gen.live, and tq2.gen.context
    ...
    For example:
        ...
        mix tq2.gen.html Accounts User users name:string
        mix tq2.gen.json Accounts User users name:string
        mix tq2.gen.live Accounts User users name:string
        mix tq2.gen.context Accounts User users name:string
