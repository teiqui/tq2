...
COPY --from=assets_deps $APP_HOME/deps/phoenix_live_view $APP_HOME/deps/phoenix_live_view
...
RUN apk add --update --no-cache ncurses imagemagick
RUN mkdir -p $APP_HOME && chown -R $USER: $APP_HOME

WORKDIR $APP_HOME

COPY --from=build $APP_HOME/_build/prod/rel/tq2 ./
